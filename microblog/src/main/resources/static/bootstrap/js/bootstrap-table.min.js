...
(function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        paginationShowPageGo:true,//扩展跳转
        search: false,
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        if (!this.options.onlyInfoPagination) {
            ...
            if(this.totalPages>= 10 && this.options.paginationShowPageGo){
                html.push('<li class="bootstrap-table-page-go"><b style="padding:0 8px;">跳转到</b>',
                              '<input style="width:50px;border:1px solid #ddd;padding:6px 5px;"/><b style="padding:0 8px;">页</b><button type="button" class="btn btn-success">确定</button></li>');
           }
            if (this.totalPages >= 6) {
...
//            $pagego.find('input').off('blur').on('blur',$.proxy(this.onPageGo,this));
            $pagego.off('click').find('button').on('click',$.proxy(this.onPageGo,this));//zyj
        }
    };
    ...
    BootstrapTable.prototype.onPageGo = function (event) {
    	//zyj
    	var numVal=$(event.currentTarget).parent().find('input').val();
    	if(isNaN(numVal)){
    		  alertModel("请输入有效数字！");
    	...
    	if(numVal<=0){
    		$(event.currentTarget).parent().find('input').val('1');
    		
    	}else{
	        if (this.options.pageNumber === +$(event.currentTarget).parent().find('input').val()) {
	            return;
	        }
    	}
    	this.options.pageNumber = +$(event.currentTarget).parent().find('input').val();
        this.updatePagination(event);
        return false;
    };
